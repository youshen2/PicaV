...
enum AppProxyProtocol: String, Codable, CaseIterable, Identifiable {
    case http
    case https
    case socks5
...
struct AppProxyConfiguration: Codable, Hashable {
...
struct AppProxyCredentials: Codable, Hashable {
    let username: String
    let password: String
...
enum AppProxyError: LocalizedError {
    ...
    case mediaBridgeUnavailable(String)
    ...
    var errorDescription: String? {
        ...
        case .mediaBridgeUnavailable(let reason):
            return "在线播放代理不可用：\(reason)"
        }
    }
}

struct AppProxyTransportError: LocalizedError, CustomNSError {
    static let errorDomain = "work.picav.app-proxy"
...
enum AppProxyConfigurationParser {

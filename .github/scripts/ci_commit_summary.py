...
"""Generate a Telegram-sized summary between successful IPA builds."""
...
BUILD_JOB_NAME = "Build unsigned IPA"
...
def github_api_get(
    ...
    request = Request(
        ...
        headers={
            ...
            "User-Agent": "PicaV-CI-Commit-Summary",
...
def format_summary(
    ...
    if previous_sha is None:
        lines.append(
            f"范围：首次 CI → CI #{current_run_number} ({current_sha[:7]})"
        )
    else:
        previous_run = (
...
def main() -> int:
    ...
    if arguments.previous_sha:
        ...
        if previous_sha is None:
            print(
                f"无法解析指定的基线 Commit：{arguments.previous_sha}",
                file=sys.stderr,
            )
            return 1
    else:
        ...
        try:
            ...
            print(
                f"::warning::{error}，将尝试使用事件 before SHA。",
                file=sys.stderr,
            )

        if previous_sha is None:
            previous_sha = fallback_baseline(
                arguments.fallback_sha,
                current_sha,
            )
